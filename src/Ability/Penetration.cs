...
using Watcher;
using static MySlugcat.Ability.ArcLightning;
...
namespace MySlugcat.Ability
{
	...
	public static class Penetration
	{
				...
				if (room != null && weapon is not ExplosiveSpear && weapon is not ScavengerBomb)
				{
					...
					if (creature != null)
					{
						...
						if (stuckInObject != creature || weaponModule.stuckInObjectTime > 3)
						{
							...
							//		result.chunk, result.onAppendagePos, Creature.DamageType.Stab, 0.25f, 60f);
							//	}
							//}
						...
						{
							weaponModule.stuckInObjectTime++;
							return false;
						}
					}
					else
					{
						weaponModule.stuckInObject = new(result.obj);
						weaponModule.stuckInObjectTime = 1;
						weaponModule.penetrateCount++;
					}
					...
					float preBonus = spear?.spearDamageBonus ?? 1f;
					float preRotationSpeed = spear?.rotationSpeed ?? 0f;
					Vector2 preRotation = spear?.rotation ?? Vector2.zero;


					result.obj.GetPenetrationModule().Penetrated = true;

					bool ret = orig_HitSomething(orig_, weapon, result, eu);
					//creature?.SetKillTag(thrownBy.abstractCreature);

					result.obj.GetPenetrationModule().Penetrated = false;
					/*if (result.obj is Lizard lizard && result.chunk != null && result.chunk.index == 0)
					{
						Vector2 attackDir = preHitVel.normalized;
						if (lizard.HitHeadShield(attackDir))
						{
							float damage = preBonus * 0.9f;
							float stunBonus = 20f;

							float num = damage / lizard.Template.baseDamageResistance;
							float num2 = ((damage * 30f) + stunBonus) / lizard.Template.baseStunResistance;

							var type = Creature.DamageType.Stab;
							if (type.Index != -1)
							{
								if (lizard.Template.damageRestistances[type.Index, 0] > 0f)
								{
									num /= lizard.Template.damageRestistances[type.Index, 0];
								}
								if (lizard.Template.damageRestistances[type.Index, 1] > 0f)
								{
									num2 /= lizard.Template.damageRestistances[type.Index, 1];
								}
							}

							num *= 1.5f;
							num2 = Mathf.Min(num2 * 2f, 120f);
							lizard.LizardState.throatHealth -= num;

							num2 *= 1f + (Mathf.InverseLerp(0.75f, 0f, lizard.LizardState.health) * UnityEngine.Random.value);
							lizard.Stun((int)num2);

							// 补伤害
							//if (lizard.State is HealthState hs)
							//	hs.health -= preBonus * 0.9f / lizard.Template.baseDamageResistance;

							// 头盾减速感
							weapon.firstChunk.vel = preHitVel * 0.7f;   // 比普通穿透更慢
						}
					}*/
					...
					if (spear != null)
					{
						spear.spearDamageBonus = preBonus * 0.9f;
						spear.rotation = preRotation;                 // 恢复朝向
						spear.rotationSpeed = preRotationSpeed;       // 恢复角速度（通常是 0）
						spear.setRotation = preHitVel.normalized;     // 下一帧对齐飞行方向
					}
		...
		public static bool HitInMouth(On.Lizard.orig_HitInMouth orig, Lizard lizard, Vector2 direction)
		{
			...
			if (module.Penetrated)
			{
				return true;
			}
			return orig(lizard, direction);
		}

		public class PenetrationModule
		{
			public bool Penetrated;
		}
		public static PenetrationModule GetPenetrationModule(this PhysicalObject obj, out PenetrationModule module)
		{
			module = GetPenetrationModule(obj);
			return module;
		}
		public static PenetrationModule GetPenetrationModule(this PhysicalObject obj)
		{
			return ModuleManager.Get(obj, c => new PenetrationModule());
		}
